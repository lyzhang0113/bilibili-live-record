...
"""
引用自https://github.com/nICEnnnnnnnLee/LiveRecorder
"""
    ...
    def dealTimeStamp(self, dest, timestamp, tagType):
        ...
        if self.lastTimestampRead[tagType] == -1:
            ...
            if timestamp > self.lastTimestampRead[tagType] + 1000:
                ...
                self.lastTimestampWrite[tagType] = timestamp - self.lastTimestampRead[tagType] + \
                                                   self.lastTimestampWrite[tagType]
        else:  # 如果出现倒序时间戳
            # 如果间隔不大，那么如实反馈
